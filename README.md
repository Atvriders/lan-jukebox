...
| `MIN_FREE_DISK_MB`       | no       | `1024`                       | Downloads are refused when the `CACHE_DIR` filesystem has less than this many MiB free, so the station can't fill the host disk. `0` disables the guard.    |
...
`MIN_FREE_DISK_MB` is a safety floor rather than a tuning knob: downloads stop
before the `CACHE_DIR` filesystem drops below that many MiB free. A full host
disk breaks **every** write (station snapshot, cookies file, logs), not just the
audio cache — keep it comfortably above your largest expected track, or set `0`
