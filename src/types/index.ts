...
// Type-only import so the `declare module "fastify"` session augmentation (near
// the bottom of this file) resolves in the main (non-test) tsc program, which
// excludes *.test.ts. Erased at emit (verbatimModuleSyntax); the `_`-prefix
// satisfies no-unused-vars. (Previously provided by src/scaffold-probe.ts.)
